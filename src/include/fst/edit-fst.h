...
class EditFst : public ImplToMutableFst<
 ...
 public:
  ...
  EditFst<Arc, WrappedFstT, MutableFstT> &operator=(const EditFst &fst) {
  ...
  static EditFst<Arc, WrappedFstT, MutableFstT> *Read(const std::string &source) {
    auto *impl = ImplToExpandedFst<Impl, MutableFst<Arc>>::Read(source);
    return impl ? new EditFst<Arc, WrappedFstT, MutableFstT>(std::shared_ptr<Impl>(impl)) : nullptr;
  }
